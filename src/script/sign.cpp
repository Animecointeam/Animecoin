...
static bool SignN(const SigningProvider& provider, const vector<valtype>& multisigdata, const BaseSignatureCreator& creator, const CScript& scriptCode, std::vector<valtype>& ret, SigVersion sigversion)
{
    int nSigned = 0;
...
static bool SignStep(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& scriptPubKey,
                     std::vector<valtype>& ret, txnouttype& whichTypeRet, SigVersion sigversion, bool route)
{
    CScript scriptRet;
    uint160 h160;
    ret.clear();
    ...
    case TX_ESCROW_CLTV:
        if (route==0)
        {
            ...
            result = SignN(provider, ms_data, creator, scriptPubKey, ret, sigversion);
            keyID = CPubKey(vSolutions[0]).GetID();
            result = Sign1(provider, keyID, creator, scriptPubKey, ret, sigversion) && result;
            ret.push_back({1});
        }
        else
        {
           ret.push_back(valtype()); // workaround CHECKMULTISIG bug
            ...
            result = SignN(provider, ms_data, creator, scriptPubKey, ret, sigversion) && result;
...
bool ProduceSignature(const SigningProvider& provider, const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata, bool route)
{
    ...
    solved = SignStep(provider, creator, script, result, whichType, SIGVERSION_BASE, route);
    bool P2SH = false;
    CScript subscript;
    ...
    if (solved && whichType == TX_SCRIPTHASH)
    {
        ...
        solved = solved && SignStep(provider, creator, script, result, whichType, SIGVERSION_BASE, route) && whichType != TX_SCRIPTHASH;
        P2SH = true;
    }
    ...
    if (solved && whichType == TX_WITNESS_V0_KEYHASH)
    {
        ...
        txnouttype subType;
        solved = solved && SignStep(provider, creator, witnessscript, result, subType, SIGVERSION_WITNESS_V0, route);
    ...
    else if (solved && whichType == TX_WITNESS_V0_SCRIPTHASH)
    {
        ...
        txnouttype subType;
        solved = solved && SignStep(provider, creator, witnessscript, result, subType, SIGVERSION_WITNESS_V0, route) && subType != TX_SCRIPTHASH && subType != TX_WITNESS_V0_SCRIPTHASH && subType != TX_WITNESS_V0_KEYHASH;
...
static Stacks CombineSignatures(const CScript& scriptPubKey, const BaseSignatureChecker& checker,
                                ...
                                Stacks sigs1, Stacks sigs2, SigVersion sigversion, bool route)
{
    switch (txType)
    {
    case TX_NONSTANDARD:
    ...
    case TX_SCRIPTHASH:
        ...
        else
        {
            ...
            Stacks result = CombineSignatures(pubKey2, checker, txType2, vSolutions2, sigs1, sigs2, sigversion, route);
    ...
    case TX_WITNESS_V0_SCRIPTHASH:
        ...
        else
        {
            ...
            sigs2.witness.clear();
            Stacks result = CombineSignatures(pubKey2, checker, txType2, vSolutions2, sigs1, sigs2, SIGVERSION_WITNESS_V0, route);
    ...
    case TX_ESCROW_CLTV:
        if (route==0)
        {
            ...
            set<valtype> allsigs;
            for (const valtype& v : sigs1.script)
            {
                if (!v.empty())
                    allsigs.insert(v);
            }
            for (const valtype& v : sigs2.script)
            {
                if (!v.empty())
                    allsigs.insert(v);
            }
            ...
            map<valtype, valtype> sigs;
            const valtype& escrow_pubkey = vSolutions[0];
            for (const valtype sig : allsigs)
            {
                if (sigs.count(escrow_pubkey))
                {
                    printf ("Escrow signature present!\n");
                    break;
                }

                if (checker.CheckSig(sig, escrow_pubkey, scriptPubKey, sigversion))
                {
                    printf ("We have a key for the escrow signature...\n");
                    sigs[escrow_pubkey] = sig;
                    break;
                }
            }

            if (sigs.count(vSolutions[0]))
            {
                printf ("Writing escrow signature...\n");
                ret.push_back(sigs[escrow_pubkey]);
            }
            else
...
SignatureData CombineSignatures(const CScript& scriptPubKey, const BaseSignatureChecker& checker,
                          const SignatureData& scriptSig1, const SignatureData& scriptSig2, bool route)
{
    txnouttype txType;
    vector<vector<unsigned char> > vSolutions;
    Solver(scriptPubKey, txType, vSolutions);

    return CombineSignatures(scriptPubKey, checker, txType, vSolutions, Stacks(scriptSig1), Stacks(scriptSig2), SIGVERSION_BASE, route).Output();
}
