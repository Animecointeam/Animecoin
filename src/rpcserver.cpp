...
CAmount AmountFromValue(const UniValue& value)
{
    ...
    CAmount amount;
    if (!ParseFixedPoint(value.getValStr(), 5, &amount)) // QA: Magic number. Rationalize 8->5 somehow.
...
UniValue stop(const UniValue& params, bool fHelp)
{
    ...
    StartShutdown();
    return "Animecoin server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        _("To use animecoind, or the -server option to animecoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> animecoin-cli " + methodname + " " + args + "\n";
}
