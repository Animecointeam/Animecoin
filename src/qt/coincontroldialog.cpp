...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::ANI;
    ...
    toolTip2 += tr("This means a fee of at least %1 per kB is required.").arg(BitcoinUnits::formatWithUnit(nDisplayUnit, GetRequiredFee(1000)));
    ...
    for (std::pair<QString, vector<COutput>> coins : mapCoins)
    {
        ...
        for (const COutput& out : coins.second)
        {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show animecoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            itemOutput->setText(COLUMN_AMOUNT_INT64, strPad(QString::number(out.tx->tx->vout[out.i].nValue), 15, " ")); // padding so that sorting works correctly
