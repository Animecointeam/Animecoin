...
void SendCoinsDialog::setModel(WalletModel *_model)
{
    ...
    if(_model && _model->getOptionsModel())
    {
        ...
        for (const int &n : confTargets) {
            ui->confTargetSelector->addItem(tr("%1 (%2 blocks)").arg(GUIUtil::formateNiceTimeOffset(n*Params().GetConsensus().nPowTargetSpacing)).arg(n));
        }
        ...
        if (settings.value("nSmartFeeSliderPosition").toInt() != 0) {
            // TODO: migrate nSmartFeeSliderPosition to nConfTarget
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Animecoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
