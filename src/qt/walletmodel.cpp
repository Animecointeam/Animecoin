...
void WalletModel::updateAddressBook(const QString &address, const QString &label,
        bool isMine, bool watchOnly, const QString &purpose, int status)
{
    if(addressTableModel)
        addressTableModel->updateEntry(address, label, isMine, watchOnly, purpose, status);
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl *coinControl)
{
    ...
    foreach(const SendCoinsRecipient &rcp, recipients)
    {
        ...
        {   // User-entered animecoin address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        {
            if (!rcp.message.isEmpty()) // Message from normal animecoin:URI (anime:123...?message=example)
...
static void NotifyAddressBookChanged(WalletModel *walletmodel, CWallet *wallet,
        const CTxDestination &address, const std::string &label, bool isMine, bool watchOnly,
        const std::string &purpose, ChangeType status)
{
    ...
    qDebug() << "NotifyAddressBookChanged : " + strAddress + " " + strLabel + " isMine=" + QString::number(isMine) + " watchOnly=" + QString::number(watchOnly)  + " purpose=" + strPurpose + " status=" + QString::number(status);
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    for (const COutPoint& outpoint : vLockedCoins)
    {
        ...
        if (nDepth < 0) continue;
        COutput out(&wallet->mapWallet[outpoint.hash], outpoint.n, nDepth, true, true);
        if (outpoint.n < out.tx->tx->vout.size() && wallet->IsMine(out.tx->tx->vout[outpoint.n]) == ISMINE_SPENDABLE && out.IsSpendableAfter(*chainActive.Tip(), *wallet)) {
    ...
    for (const COutput& out : vCoins)
    {
        ...
        if(!out.IsSpendableAfter(*chainActive.Tip(), *wallet) || !ExtractDestination(cout.tx->tx->vout[cout.i].scriptPubKey, address)) {
            continue;
        }
...
{
    emit spvEnabledStatusChanged(fSPVModeEnabled);
}
