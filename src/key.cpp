...
public:
    ...
    bool Sign(const uint256 &hash, std::vector<unsigned char>& vchSig) {
        ...
        if (BN_cmp(sig_s, halforder) > 0) {
            // enforce low S values, by negating the value (modulo the order) if above order/2. Updated for OpenSSL-1.1, testing required.
            #if OPENSSL_VERSION_NUMBER > 0x1000ffffL
            BIGNUM *nc_sig_s=BN_new();
            BN_sub(nc_sig_s, order, sig_s);
            BIGNUM *nc_sig_r=BN_new();
            BN_copy (nc_sig_r,sig_r);
            ECDSA_SIG_set0(sig, nc_sig_r, nc_sig_s);
            #else
    ...
    bool SignCompact(const uint256 &hash, unsigned char *p64, int &rec) {
        ...
        if (nBitsR <= 256 && nBitsS <= 256) {
            ...
            assert(fOk);
            BN_bn2bin(sig_r,&p64[32-(nBitsR+7)/8]); // Updated for OpenSSL-1.1
