    ...
    if (!tx.IsCoinBase())
    {
        ...
        if (fScriptChecks) {
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                // Verify signature
                CScriptCheck check(scriptPubKey, amount, tx, i, flags, cacheStore, &txdata); // and &txdata for witness.
                if (pvChecks) {
 ...
 *
 * If FLUSH_STATE_NONE is used, then FlushStateToDisk(...) won't do anything
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
...
static bool AcceptBlock(const std::shared_ptr<const CBlock>& pblock, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const CDiskBlockPos* dbp, bool* fNewBlock, bool onlyHeaderCheck = false)
{
    ...
    if (onlyHeaderCheck) {
        LogPrint(BCLog::NET, "Accept specific block %s (%d)\n", pindex->GetBlockHash().ToString(), pindex->nHeight);
...
bool ProcessNewBlock(const CChainParams& chainparams, const std::shared_ptr<const CBlock> pblock, bool fForceProcessing, bool *fNewBlock, std::shared_ptr<CAuxiliaryBlockRequest> blockRequest)
{
...

    {
        ...
        if (ret) {
            // Store to disk
            ret = AcceptBlock(pblock, state, chainparams, &pindex, fForceProcessing, nullptr, fNewBlock, (blockRequest != nullptr));
        }
        CheckBlockIndex(chainparams.GetConsensus());
        if (!ret) {
...
static void FindFilesToPrune(std::set<int>& setFilesToPrune, uint64_t nPruneAfterHeight)
{
    ...
    LogPrint(BCLog::PRUNE, "Prune: target=%dMiB actual=%dMiB diff=%dMiB min_must_keep=%d removed %d blk/rev pairs\n",
