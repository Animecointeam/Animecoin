...
bool MarkBlockAsInFlight(NodeId nodeid, const uint256& hash, const Consensus::Params& consensusParams, CBlockIndex *pindex = nullptr, list<QueuedBlock>::iterator **pit = nullptr) {
    ...
    list<QueuedBlock>::iterator it = state->vBlocksInFlight.insert(state->vBlocksInFlight.end(),
            {hash, pindex, pindex != NULL, std::unique_ptr<PartiallyDownloadedBlock>(pit ? new PartiallyDownloadedBlock(&mempool) : nullptr)});
    state->nBlocksInFlight++;
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                if (mapBlockIndex.count(hash) == 0 || (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA) == 0) {
                    LOCK(cs_main);
                    CValidationState state;
                    if (AcceptBlock(block, state, chainparams, NULL, true, dbp, nullptr))
                        nLoaded++;
                    if (state.IsError())
                        break;
                ...
                if (send && (mi->second->nStatus & BLOCK_HAVE_DATA))
                {
                    ...
                    else if (inv.type == MSG_CMPCT_BLOCK)
                    {
                        ...
                        if (CanDirectFetch(consensusParams) && mi->second->nHeight >= chainActive.Height() - MAX_CMPCTBLOCK_DEPTH) {
                            ...
                            connman.PushMessage(pfrom, NetMsgType::CMPCTBLOCK, cmpctblock);
                        } else
                            connman.PushMessage(pfrom, NetMsgType::BLOCK, block);
                    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman& connman)
{
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if (CanDirectFetch(chainparams.GetConsensus()) &&
                        ...
                        // if (nodestate->fProvidesHeaderAndIDs && !(pfrom->GetLocalServices() & NODE_XYZ))
                        if (nodestate->fProvidesHeaderAndIDs)
    ...
    else if (strCommand == NetMsgType::GETBLOCKTXN)
    {
        ...
        connman.PushMessage(pfrom, NetMsgType::BLOCKTXN, resp);
    }
    ...
    else if (strCommand == NetMsgType::CMPCTBLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        CBlockIndex *pindex = nullptr;
        CValidationState state;
        if (!AcceptBlockHeader(cmpctblock.header, state, &pindex)) {
            int nDoS;
            if (state.IsInvalid(nDoS)) {
                if (nDoS > 0)
                    Misbehaving(pfrom->GetId(), nDoS);
    ...
    else if (strCommand == NetMsgType::BLOCKTXN && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        if (fBlockRead) {
            ...
            ProcessNewBlock(chainparams, &block, true, nullptr, &fNewBlock);
            if (fNewBlock)
    ...
    else if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && chainActive.Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            if (!chainActive.Contains(pindexWalk)) {
                ...
                for (CBlockIndex *pindex : reverse_iterate(vToFetch)) {
    ...
    else if (strCommand == NetMsgType::BLOCK && !fImporting && !fReindex) // Ignore blocks received while importing
    {
        ...
        bool fNewBlock = false;
        ProcessNewBlock(chainparams, &block, forceProcessing, nullptr, &fNewBlock);
        if (fNewBlock)
            pfrom->nLastBlockTime = GetTime();
    }
        ...
        //
        {
            ...
            if (!fRevertToInv && !vHeaders.empty()) {
                if (vHeaders.size() == 1 && state.fPreferHeaderAndIDs) {
                    ...
                    CBlockHeaderAndShortTxIDs cmpctblock(block);
                    connman.PushMessage(pto, NetMsgType::CMPCTBLOCK, cmpctblock);
