    ...
    unsigned int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& coin : setCoins)
                {
                    ...
                    const CScript& script = coin.first->tx->vout[coin.second].scriptPubKey;
                    bool failure;
                    int64_t cltv_height, cltv_time;
        ...
        if (sign)
        {
            ...
            for (const auto& coin : setCoins)
            {
                ...
                SignatureData sigdata;
                if (!ProduceSignature(TransactionSignatureCreator(this, &txNewConst, nIn, coin.first->tx->vout[coin.second].nValue, SIGHASH_ALL), scriptPubKey, sigdata, 1))
                {
                    strFailReason = _("Signing transaction failed");
                    return false;
                } else {
...
bool CWallet::ParameterInteraction()
{
    ...
    {
        CAmount n = 0;
        if (!ParseMoney(GetArg("-mintxfee", ""), n))
...
bool CWalletTx::AcceptToMemoryPool(bool fLimitFree, CAmount nAbsurdFee, CValidationState& state)
{
    ...
    bool ret = ::AcceptToMemoryPool(mempool, state, tx, fLimitFree, nullptr, nullptr, false, nAbsurdFee);
    fInMempool = ret;
    return ret;
}
