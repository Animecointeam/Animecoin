        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
                ...
                // Limit size
                if (nBytes >= MAX_STANDARD_TX_SIZE) // if ((nBytes + 4000 >= MAX_STANDARD_TX_SIZE) || (wtxNew.vin.size() >= 160)) Anime: Added safety margin 4000 bytes and 160 transactions
                {
...
bool CWallet::InitLoadWallet()
{
    ...
    if (fFirstRun)
    {
        ...
        if (!walletInstance->hdChain.masterKeyID.IsNull() && !useHD)
            return UIError(strprintf(_("Error loading %s: You can't disable HD on a already existing HD wallet"), walletFile));
        if (walletInstance->hdChain.masterKeyID.IsNull() && useHD)
            return UIError(strprintf(_("Error loading %s: You can't enable HD on a already existing non-HD wallet"), walletFile));
    }
