...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw runtime_error(
                ...
                "\nExamples:\n"
                + HelpExampleCli("addnode", "\"192.168.0.6:1212\" \"onetry\"")
                + HelpExampleRpc("addnode", "\"192.168.0.6:1212\", \"onetry\"")
            );
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            ...
            "(note that onetry addnodes are not listed here)\n"
                ...
                "       {\n"
                "         \"address\" : \"192.168.0.201:1212\",  (string) The animecoin server IP and port we're connected to\n"
...
UniValue makekeypair(const JSONRPCRequest& request)
{
    ...
    result.pushKV("PublicKey", HexStr(pubkey.begin(), pubkey.end()));
    result.pushKV("PrivateKey", CBitcoinSecret(key).ToString());
    return result;
}
