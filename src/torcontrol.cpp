...
void TorControlConnection::eventcb(struct bufferevent *bev, short what, void *ctx)
{
    ...
    if (what & BEV_EVENT_CONNECTED) {
        LogPrint(BCLog::TOR, "tor: Succesfully connected!\n");
        self->connected(*self);
...
{
    if (reply.code == 250) {
        LogPrint(BCLog::TOR, "tor: ADD_ONION succesful\n");
...
{
    if (reply.code == 250) {
        LogPrint(BCLog::TOR, "tor: Authentication succesful\n");
...
{
    if (reply.code == 250) {
        LogPrint(BCLog::TOR, "tor: SAFECOOKIE authentication challenge succesful\n");
