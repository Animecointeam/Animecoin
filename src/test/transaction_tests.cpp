...
SignatureData CombineSignatures(const CMutableTransaction& input1, const CMutableTransaction& input2, const CTransactionRef tx)
{
    ...
    sigdata = DataFromTransaction(input1, 0, tx->vout[0], 1);
    sigdata.MergeSignatureData(DataFromTransaction(input2, 0, tx->vout[0], 1));
    ProduceSignature(DUMMY_SIGNING_PROVIDER, MutableTransactionSignatureCreator(&input1, 0, tx->vout[0].nValue), tx->vout[0].scriptPubKey, sigdata, 1);
    return sigdata;
}
