...
private:
    ...
    void init()
    {
        ...
        self = BN_new();
        if (!self)
            throw bignum_error("CBigNum::init() : BN_new() returned nullptr");
    }
...
public:
    ...
    void setuint64(unsigned long long n)
    {
    ...
    {
        unsigned int nSize = BN_bn2mpi(self, nullptr);
        if (nSize < 4)
            return 0;
...
inline const CBigNum operator/(const CBigNum& a, const CBigNum& b)
{
    CAutoBN_CTX pctx;
    CBigNum r;
    if (!BN_div(r.get(), nullptr, a.cget(), b.cget(), pctx))
